...
class Bert(nn.Module):
    ...
    def get_contextualized(self, input_ids: torch.LongTensor, attention_mask: torch.BoolTensor) -> torch.Tensor:
        ...
        contextualized_embeddings: torch.Tensor = self.transformer(static_embeddings, attention_mask.unsqueeze(1), relative_embedding)

        return contextualized_embeddings

    def forward(self, input_ids: torch.LongTensor, attention_mask: torch.BoolTensor, masked_lm_labels: torch.LongTensor | None = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        gold_labels: torch.Tensor = masked_lm_labels.flatten()
        gold_labels = gold_labels[gold_labels != -100]

        loss: torch.Tensor = F.cross_entropy(logits, gold_labels, reduction="none")

        with torch.no_grad():
            accuracy: torch.Tensor = (logits.argmax(-1) == gold_labels).float().mean()
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.BoolTensor, relative_embedding: torch.Tensor) -> torch.Tensor:
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.BoolTensor, relative_embedding: torch.Tensor) -> torch.Tensor:
        attention: torch.Tensor
        mlp: torch.Tensor

        attention: torch.Tensor = self.attention(x, attention_mask, relative_embedding)
        x = x + attention
...
class Attention(nn.Module):
    ...
    def __init__(self, config: argparse.Namespace) -> None:
        ...
        if config.qk_size % config.num_attention_heads != 0:
            raise ValueError(f"The hidden size {config.qk_size} is not a multiple of the number of attention heads {config.num_attention_heads}")
        if config.v_size % config.num_attention_heads != 0:
            raise ValueError(f"The hidden size {config.v_size} is not a multiple of the number of attention heads {config.num_attention_heads}")
        if config.qk_size + config.v_size != config.hidden_size:
            raise ValueError(f"The sum of {config.qk_size} and {config.v_size} must be hidden size {config.hidden_size}")

        self.hidden_size: int = config.hidden_size
        self.qk_size: int = config.qk_size
        self.v_size: int = config.v_size
        self.num_heads: int = config.num_attention_heads
        self.qk_head_size: int = config.qk_size // config.num_attention_heads
        self.v_head_size: int = config.v_size // config.num_attention_heads
        self.position_bucket_size: int = config.position_bucket_size

        self.in_proj_qk: nn.Linear = nn.Linear(config.qk_size, 2*config.qk_size, bias=True)
        self.in_proj_v: nn.Linear = nn.Linear(config.v_size, config.v_size, bias=True)
        self.out_proj: nn.Linear = nn.Linear(config.v_size, config.hidden_size, bias=True)

        self.qk_pre_layer_norm: nn.LayerNorm = nn.LayerNorm(config.qk_size, config.layer_norm_eps, elementwise_affine=False)
        self.v_pre_layer_norm: nn.LayerNorm = nn.LayerNorm(config.v_size, config.layer_norm_eps, elementwise_affine=False)
        self.qk_post_layer_norm: nn.LayerNorm = nn.LayerNorm(config.qk_size, config.layer_norm_eps, elementwise_affine=True)
        self.v_post_layer_norm: nn.LayerNorm = nn.LayerNorm(config.v_size, config.layer_norm_eps, elementwise_affine=True)
        ...
        self.scale: float = 1.0 / math.sqrt(3 * self.qk_head_size)
        self.initialize()
    ...
    def create_relative_positioning(self, relative_embedding: torch.Tensor, sequence_length: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        relative_query_pos = relative_query_pos.view(sequence_length, sequence_length, self.num_heads, self.qk_head_size)
        relative_key_pos = relative_key_pos.view(sequence_length, sequence_length, self.num_heads, self.qk_head_size)
    ...
    def calculate_attention_score(self, query: torch.Tensor, key: torch.Tensor, relative_query_pos: torch.Tensor, relative_key_pos: torch.Tensor, sequence_length: int, batch_size: int) -> torch.Tensor:
        query = query.reshape(sequence_length, batch_size * self.num_heads, self.qk_head_size).transpose(0, 1)
        key = key.reshape(sequence_length, batch_size * self.num_heads, self.qk_head_size).transpose(0, 1)
        attention_scores: torch.Tensor = torch.bmm(query, key.transpose(1, 2) * self.scale)

        query = query.view(batch_size, self.num_heads, sequence_length, self.qk_head_size)
        key = key.view(batch_size, self.num_heads, sequence_length, self.qk_head_size)
    ...
    def calculate_output(self, attention_probabilities: torch.Tensor, value: torch.Tensor, sequence_length: int, batch_size: int) -> torch.Tensor:
        context: torch.Tensor
        v_context: torch.Tensor

        value = value.view(sequence_length, batch_size * self.num_heads, self.v_head_size).transpose(0, 1)
        output: torch.Tensor = torch.bmm(attention_probabilities.flatten(0, 1), value)  # shape: [B*H, Q, D]
        output = output.transpose(0, 1).reshape(output.size(1), -1, self.v_size)  # shape: [Q, B, H*D]
        context, v_context = self.out_proj(output).tensor_split((self.qk_size,), dim=-1)
        context = self.dropout(self.qk_post_layer_norm(context))
        v_context = self.dropout(self.v_post_layer_norm(v_context))

        return torch.cat([context, v_context], dim=-1)
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.BoolTensor, relative_embedding: torch.Tensor) -> torch.Tensor:
        ...
        relative_query_pos: torch.Tensor
        relative_key_pos: torch.Tensor
        qk_hidden_states: torch.Tensor
        v_hidden_states: torch.Tensor

        sequence_length, batch_size, _ = hidden_states.size()

        qk_hidden_states, v_hidden_states = hidden_states.tensor_split((self.qk_size,), dim=-1)

        if self.position_indices.size(0) < sequence_length:
            self.register_position_indices(sequence_length)

        qk_hidden_states = self.qk_pre_layer_norm(qk_hidden_states)
        v_hidden_states = self.v_pre_layer_norm(v_hidden_states)

        query, key = self.in_proj_qk(qk_hidden_states).chunk(2, dim=-1)
        ...
        context: torch.Tensor = self.calculate_output(attention_probabilities, value, sequence_length, batch_size)
...
class FusedAttention(Attention):
    ...
    def __init__(self, config: argparse.Namespace) -> None:
        ...
        self.out_proj: nn.Sequential = nn.Sequential(
            nn.Linear(config.v_size, 2*config.hidden_size, bias=False),
            GeGLU(),
    ...
    def initialize_new(self) -> None:
        ...
        nn.init.trunc_normal_(self.out_proj[0].weight, mean=0.0, std=std, a=-2*std, b=2*std)
        nn.init.trunc_normal_(self.out_proj[-1].weight, mean=0.0, std=std, a=-2*std, b=2*std)

        self.out_proj[-1].bias.data.zero_()
...
class Embedding(nn.Module):
    ...
    def __init__(self, config: argparse.Namespace) -> None:
        ...
        self.word_embedding: nn.Embedding = nn.Embedding(config.vocab_size, config.hidden_size)
        self.dropout: nn.Dropout = nn.Dropout(config.hidden_dropout_prob)
        self.scale: float = math.sqrt(config.hidden_size)

        self.relative_embedding: nn.Parameter = nn.Parameter(torch.zeros(2 * config.position_bucket_size - 1, config.qk_size))
        self.relative_layer_norm: nn.LayerNorm = nn.LayerNorm(config.qk_size, eps=config.layer_norm_eps)
    ...
    def forward(self, input_ids: torch.LongTensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        word_embedding: torch.Tensor = self.dropout(self.word_embedding(input_ids) * self.scale)
        relative_embedding: torch.Tensor = self.relative_layer_norm(self.relative_embedding)

        return word_embedding, relative_embedding
