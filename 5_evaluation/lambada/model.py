...
class Bert(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, masked_lm_labels=None):
        ...
        loss = F.cross_entropy(logits, gold_labels, reduction="none").mean()
        z_loss = torch.logsumexp(logits, dim=-1).pow(2).mean()

        with torch.no_grad():
            accuracy = (logits.argmax(-1) == gold_labels).float().mean()

        num_tokens = gold_labels.size(0)

        return logits, loss, accuracy, z_loss, num_tokens
