...
def parse_arguments():
    ...
    parser.add_argument("--name", default="SinLU", type=str, help="Name of the run.")
...
def prepare_model_and_optimizer(args):
    ...
    decay_params = [(n, p) for n, p in model.named_parameters() if p.requires_grad and p.dim() >= 2]
    no_decay_params = [(n, p) for n, p in model.named_parameters() if p.requires_grad and p.dim() < 2]
