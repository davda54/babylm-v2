...
class Mogrifier(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.gate_a = nn.Sequential(nn.Linear(config.intermediate_size, 64, bias=False), nn.Linear(64, config.intermediate_size))
        self.gate_b = nn.Sequential(nn.Linear(config.intermediate_size, 64, bias=False), nn.Linear(64, config.intermediate_size))
        self.rounds = 3
    ...
    def forward(self, x):
        ...
        for _ in range(self.rounds):
            a = a * F.gelu(self.gate_a(b))
            b = b * F.gelu(self.gate_b(a))
