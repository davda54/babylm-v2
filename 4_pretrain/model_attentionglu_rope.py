...
class AttentionGLU(nn.Module):
    def __init__(self, config):
        ...
        self.rotary_emb = RotaryEmbedding(self.qk_head_size)

        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)
        self.out_dropout = nn.Dropout(config.hidden_dropout_prob)
        self.scale = 1.0 / math.sqrt(self.qk_head_size)
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        cos, sin = self.rotary_emb(value, torch.arange(query_len, device=hidden_states.device).expand(batch_size * self.num_heads, -1))
