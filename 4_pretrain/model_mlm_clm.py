...
class EncoderModel(nn.Module):
    def __init__(self, config, causal_mult=3):
        super().__init__()
    ...
    def forward(self, x, x_mask, masked_lm_labels=None, causal_pos=None, causal_ratio=None, mlm_ratio=None, only_causal=False, only_mlm=False):

        if len(x_mask.shape) == 2:
            x_mask = x_mask.unsqueeze(1).unsqueeze(2)
        elif len(x_mask.shape) == 3:
            x_mask = x_mask.unsqueeze(1)

        static_embeddings, relative_embedding = self.embedding(x)
        contextualized_embeddings = self.transformer(static_embeddings, x_mask, relative_embedding)
        
        if causal_pos is not None:
            
            causal_loss = torch.zeros(1)
            mlm_loss = torch.zeros(1)
            causal_accuracy = torch.tensor(0.0)
            causal_perplexity = torch.tensor(0.0)
            mlm_accuracy = torch.tensor(0.0)
            mlm_perplexity = torch.tensor(0.0)


            if sum(causal_pos):
                causal_contextualized_embeddings = torch.index_select(contextualized_embeddings, 1, torch.nonzero(causal_pos).squeeze())
                causal_masked_lm_labels = torch.index_select(masked_lm_labels, 1, torch.nonzero(causal_pos).squeeze())

                causal_subword_prediction = self.classifier(causal_contextualized_embeddings, causal_masked_lm_labels)

                causal_gold_labels = causal_masked_lm_labels.flatten()
                ...
                with torch.no_grad():
                    ...
                    causal_perplexity = torch.exp(causal_loss)

            if sum(~causal_pos):
                mlm_contextualized_embeddings = torch.index_select(contextualized_embeddings, 1, torch.nonzero(~causal_pos).squeeze())
                mlm_masked_lm_labels = torch.index_select(masked_lm_labels, 1, torch.nonzero(~causal_pos).squeeze())

                mlm_subword_prediction = self.classifier(mlm_contextualized_embeddings, mlm_masked_lm_labels)

                mlm_gold_labels = mlm_masked_lm_labels.flatten()
                mlm_gold_labels = mlm_gold_labels[mlm_gold_labels != -100]
                mlm_loss = F.cross_entropy(mlm_subword_prediction, mlm_gold_labels)

                with torch.no_grad():
                    mlm_accuracy = (mlm_subword_prediction.argmax(-1) == mlm_gold_labels).float().mean()
                    mlm_perplexity = torch.exp(mlm_loss)

            if only_causal:
                return causal_loss
            elif only_mlm:
                return mlm_loss
            if sum(causal_pos) and sum(~causal_pos):
                if causal_ratio is None:
                    loss = (self.causal_mult*causal_loss + mlm_loss) / (self.causal_mult + 1)
                else:
                    loss = (causal_ratio * causal_loss + mlm_ratio * mlm_loss) / (causal_ratio + mlm_ratio)
            elif sum(~causal_pos):
                loss = mlm_loss
            else:
                loss = causal_loss

            return loss, causal_loss.item(), mlm_loss.item(), causal_perplexity, mlm_perplexity, causal_accuracy, mlm_accuracy
