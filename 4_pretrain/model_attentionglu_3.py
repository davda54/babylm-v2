...
class AttentionGLU(nn.Module):
    def __init__(self, config):
        ...
        self.value_projection = nn.Linear(config.hidden_size, config.intermediate_size + config.hidden_size, bias=False)
    ...
    def forward(self, hidden_states, attention_mask, relative_embedding):
        ...
        value, gate = F.gelu(self.value_projection(hidden_states)).tensor_split([self.hidden_size], dim=-1)  # shape: [T, B, D], [T, B, I]
        ...
        context = torch.einsum("bhqk,kbd->qbhd", attention_probs, value).flatten(2, 3)  # shape: [Q, B, I]
